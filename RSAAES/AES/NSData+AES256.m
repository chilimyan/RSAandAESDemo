...
size_t const kAlgorithmKeySize = kCCKeySizeAES128;
...
+ (NSString *)AES128EncryptWithPlainText:(NSString *)plain key:(NSString*)key{
    NSString *const kInitVector = @"16-Bytes--String";
    NSData *contentData = [plain dataUsingEncoding:NSUTF8StringEncoding];
    NSUInteger dataLength = contentData.length;
    // 为结束符'\\0' +1
    char keyPtr[kAlgorithmKeySize + 1];
...
+ (NSString *)AES128DecryptWithCiphertext:(NSString *)ciphertexts key:(NSString*)key{
    ...
    NSData *contentData = [[NSData alloc] initWithBase64EncodedString:ciphertexts options:NSDataBase64DecodingIgnoreUnknownCharacters];
    NSUInteger dataLength = contentData.length;
    char keyPtr[kAlgorithmKeySize + 1];
    memset(keyPtr, 0, sizeof(keyPtr));
    [key getCString:keyPtr maxLength:sizeof(keyPtr) encoding:NSUTF8StringEncoding];
...
- (NSString *)base64Encoding
{
    ...
    return [[[NSString alloc] initWithBytesNoCopy:characters length:length encoding:NSUTF8StringEncoding freeWhenDone:YES] init];
}
@end
